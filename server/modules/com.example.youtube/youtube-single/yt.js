DSCMSView.run = function(DSCMSViewTools) {
  ...
  var vidId = $.grep(DSCMSViewTools.myConfig.configItems, function(item) {
    return item.key === "videoId";
  })[0].value;
  ...
  function newYoutubePlayer(){
	  player = new YT.Player(DSCMSViewTools.myWindows['Youtube single video player'], {
      height: '100%',
      width: '100%',
	  playerVars : {
                  'autoplay' : 1,
